...
export function calculateStats(arr) {
  if (!arr || arr.length === 0) {
    return { mean: 0, median: 0, stdDev: 0, count: 0, histogram: {} };
  }
  const mean = (arr.reduce((a, b) => a + b, 0) / arr.length).toFixed(2);
  const med = median(arr).toFixed(2);
  const sd = standardDeviation(arr).toFixed(2);

  // Build histogram: bucket by rounded value
  const histogram = {};
  arr.forEach((v) => {
    const key = Math.round(v);
  ...
  return { mean, median: med, stdDev: sd, count: arr.length, histogram };
}
